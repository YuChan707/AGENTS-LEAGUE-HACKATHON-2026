...
export default function ProjectSettings({
  ...
  const btnLabel =
    mode === "dashboard" ? "Save Configuration" : "Update";
  ...
  return (
    <section className="flex flex-col gap-[var(--sp-sm)]">
      ...
      {/* Settings form card */}
      <div
        className="fl-card p-[var(--sp-lg)] flex flex-col gap-[var(--sp-lg)]"
      >
        {/* Type of Audience */}
        <div className="flex flex-col gap-[var(--sp-xs)]">
          <FieldLabel>Type of audience</FieldLabel>
          <div className="fl-input">
            <select defaultValue="" className="bg-transparent border-none p-0 pb-[var(--sp-xs)] focus:ring-0 w-full appearance-none cursor-pointer text-[length:var(--text-body)]" style={{ color: "var(--color-on-surface)" }}>
        ...
        {showUpdateButton && (
          <button
            onClick={onUpdate}
            className="fl-btn-primary mt-[var(--sp-md)] w-full py-[var(--sp-sm)] shadow-sm uppercase tracking-widest"
          >
            {btnLabel}
          </button>
        )}
